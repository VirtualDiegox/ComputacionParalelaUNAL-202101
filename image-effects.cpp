...
//Estructura para pasar informacion a los filtros por pthread 
struct filter_data{ 
    int inicio;
    int fin;
    Mat Ptr_src;
...
//filtro 1 POSIX 
void *grisPromedio(void *threadarg){

    struct filter_data *my_data;
    

    my_data = (struct filter_data*)threadarg;

    //sacamos la info de la struct para utilizarla
    Mat Ptr_src = my_data->Ptr_src;
    Mat Ptr_dst = my_data->Ptr_dst;
...
//filtro 1 secuencial 
void grisPromedio(Mat Ptr_src,Mat Ptr_dst){
...
//filtro 2 secuencial 
void grisLuma(Mat Ptr_src,Mat Ptr_dst){
    int width = Ptr_src.rows;
    int height = Ptr_dst.cols;
...
//filtro 3 POSIX 
void *sombrasDeGris(void *threadarg){
    struct filter_data *my_data;
...
//filtro 3 secuencial
void sombrasDeGris(Mat Ptr_src,Mat Ptr_dst, int capas){
...
//filtro 4 secuencial 
void granular(Mat Ptr_src,Mat Ptr_dst, int capas){
    if(capas<2) capas = 2;
    if(capas>255) capas = 255;
    ...
    if(hilos==1){
        ...
        switch(parametro_filtro){
            ...
            case 1:
                ...
                grisPromedio(imagen_src,image_dst);
            ...
            case 3:
                gettimeofday(tval_before, NULL);
                sombrasDeGris(imagen_src,image_dst,capas);
                gettimeofday(tval_after, NULL);
                timersub(tval_after, tval_before, tval_result);
                printf("%ld.%06ld", (long int)tval_result->tv_sec, (long int)tval_result->tv_usec);
                break;
